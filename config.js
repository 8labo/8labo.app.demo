...
(function () {
  ...
  const addStaffAdminEntry = async () => {
    try {
      ...
      link.href = cfg.adminUrls.COMMON;
      link.textContent = '管理';
      link.setAttribute('aria-label', 'スタッフ管理ポータル');
      link.style.cssText = [
        'position:fixed','right:12px','bottom:10px','z-index:20',
        'font-size:9px','font-weight:700','letter-spacing:.06em',
        'color:#9ca3af','text-decoration:none','opacity:.72',
        'padding:5px 7px','border-radius:8px','background:rgba(255,255,255,.78)',
        'border:1px solid rgba(229,231,235,.8)','backdrop-filter:blur(8px)'
      ].join(';');
